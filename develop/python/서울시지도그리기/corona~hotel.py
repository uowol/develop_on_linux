...
df_hotel_raw = pd.read_excel("hotel.xlsx").drop(["Unnamed: 0"], axis=1)
for i in range(len(df_hotel_raw)):
    pos = df_hotel_raw.iloc[i][2]
    if pos is np.nan:
        pos = min_dist(df_hotel_raw.x[i], df_hotel_raw.y[i])['name']
    else:
        pos = pos.split()[1]
    df_hotel_raw.pos[i] = pos

se_pos_value = df_hotel_raw.pos.value_counts()
df_hotel = pd.DataFrame([se_pos_value.index, se_pos_value]).drop(25, axis=1).transpose()
df_hotel.columns = ['name', 'value']
df_hotel = df_hotel.sort_values(by=['name']).reset_index(drop=True)
# display(df_hotel)
print(df_hotel)
exit()
