...
df_motel = pd.read_excel("motel.xlsx").drop(["Unnamed: 0"], axis=1)
...
for i in range(len(df_motel)):
    pos = df_motel.iloc[i][2]
    if pos is np.nan:
        pos = min_dist(df_motel.x[i], df_motel.y[i])['name']
    else:
        pos = pos.split()[1]
    df_motel.pos[i] = pos

se_pos_value1 = df_motel.pos.value_counts()

state_data1 = pd.DataFrame([se_pos_value1.index, se_pos_value1]).transpose()
state_data1.columns = ['name', 'value']
state_data1 = state_data1.sort_values(by=['name']).reset_index(drop=True)

se_pos_value2 = df_hotel.pos.value_counts()

state_data2 = pd.DataFrame([se_pos_value2.index, se_pos_value2]).drop(25, axis=1).transpose()
state_data2.columns = ['name', 'value']
state_data2 = state_data2.sort_values(by=['name']).reset_index(drop=True)

state_data = pd.DataFrame([state_data1.name, state_data1.value + state_data2.value]).transpose()
state_data.columns = ['name', 'value']
# print(state_data1, state_data1.name, state_data)
# exit()

# state_data1 = state_data1.sort_values(by=['name']).reset_index(drop=True)

df['value'] = state_data1.value + state_data2.value
...
# Save to html
file_name = "hotel_plus_motel.html"
m.save(file_name)
