...
df_oldmen = pd.read_csv("oldmen.csv", encoding='euc-kr').drop(["Unnamed: 0"], axis=1)
df_oldmen = df_oldmen.drop(0).reset_index(drop=True)    # '전체' 제거
df_oldmen.columns = ["name", "value"]
# print(df_oldmen)
# exit()

state_data = df_oldmen.sort_values(by=['name']).reset_index(drop=True)
