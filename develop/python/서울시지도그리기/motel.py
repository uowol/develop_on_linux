...
for i in range(len(df_motel)):
    ...
    if pos is np.nan:
        ...
        pos = pos.split()[1]
    df_motel.pos[i] = pos
# print(cnt, df[df.name == '강남구'])

se_pos_value = df_motel.pos.value_counts()
se_pos_rate = se_pos_value / se_pos_value.sum()

state_data = pd.DataFrame([se_pos_value.index, se_pos_value]).transpose()
state_data.columns = ['name', 'value']
state_data = state_data.sort_values(by=['name']).reset_index(drop=True)
